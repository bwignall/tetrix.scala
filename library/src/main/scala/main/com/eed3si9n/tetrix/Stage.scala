...
object Stage {
  ...
  def randomStream(random: util.Random): LazyList[PieceKind] =
  ...
  def newState(
    ...
    val withNext =
      spawn(GameState(Nil, gridSize, dummy, dummy, kinds)).copy(blocks = blocks)
    spawn(withNext)
  }
  ...
  val drop: GameState => GameState = (s0: GameState) =>
    Function.chain(
      (Nil padTo (s0.gridSize._2, transit { _.moveBy(0.0, -1.0) })) ++
        List(tick)
    )(s0)
      ...
      @tailrec def tryRow(i: Int, s: GameState): GameState =
        if (i < 0) s
          ...
          tryRow(
            i - 1,
            s.copy(
              blocks = (s.blocks filter { _.pos._2 < i }) ++
  ...
  private[this] def transit(
    ...
    (s: GameState) =>
      s.status match {
        case ActiveStatus =>
          validate(
            s.unload(s.currentPiece)
              .copy(currentPiece = trans(s.currentPiece), lastDeleted = 0)
          ) map {
            case x =>
              x.load(x.currentPiece)
          } getOrElse { onFail(s) }
  ...
  private[this] def validate(s: GameState): Option[GameState] = {
    ...
    if (
      (currentPoss forall inBounds) &&
      (s.blocks map { _.pos } intersect currentPoss).isEmpty
    ) Some(s)
    else None
  }
