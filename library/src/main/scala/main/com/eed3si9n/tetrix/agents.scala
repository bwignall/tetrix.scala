...
class Agent {
  ...
  def penalty(s: GameState): Double = {
    val groupedByX = s.unload(s.currentPiece).blocks map { _.pos } groupBy {
      _._1
    }
    ...
    val crevasses = (-1 to s.gridSize._1 - 2) flatMap { x =>
      ...
      if (down < -3 && up > 3)
        Some(
          math.min(
            crevassesWeight * hWithDefault(x),
            crevassesWeight * hWithDefault(x + 2)
          )
        )
      else None
    }
    val coverupWeight = 1
    val coverups = groupedByX flatMap {
      case (k, vs) =>
